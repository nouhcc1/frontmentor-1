  ...
  return (//hover:shadow-xl transition-shadow duration-300 shadow-sm shadow-slate-500
    <div className="p-1 max-w-sm overflow-hidden ">
      ...
      src={product.image.desktop} alt={product.name} width={750} height={750}/>
        ...
        {count > 0  
            ? <button className="px-2  transform -translate-y-1/2  w-2/3 py-3 rounded-full grid grid-cols-6 items-center place-items-center bg-redbutton " onClick={handleClick}>
            ...
            : <button 
                  className="outline outline-1 outline-slate-300 outline- transform -translate-y-1/2 w-2/3 rounded-full bg-white  py-3 flex items-center justify-center hover:outline hover:outline-2 hover:outline-amber-600 hover:text-amber-600"
                  onClick={handleClick}
                >
